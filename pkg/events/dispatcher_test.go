...
func (e *TestEvent) GetError() error {
	return nil
}
...
func (eh *TestEventHandler) Handle(event EventInterface, wg *sync.WaitGroup) {
	defer wg.Done()
}
...
func (suite *EventDispatcherTestSuite) Has() {
...
func (suite *EventDispatcherTestSuite) Clear() {
	// Event 1
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
...
func (eh *MockHandler) Handle(event EventInterface, wg *sync.WaitGroup) {
	eh.Called(event)
	wg.Done()
